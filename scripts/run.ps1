...
if ($Local) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "dotnet run --project src/DailyNotes.Api/DailyNotes.Api.csproj"
}
else {
...
if (Test-Path "src/daily-notes-ui") {
    ...
    Start-Process powershell -WorkingDirectory "src/daily-notes-ui" -ArgumentList "-NoExit", "-Command", "npm run dev"
}
else {
